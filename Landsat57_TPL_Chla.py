# -*- coding: utf-8 -*-
"""
Created on Mon Nov 20 08:41:12 2023
...
FilePath = r"J:\979个湖泊处理\Landsat5" 
file_names = os.listdir(FilePath)
SavePath = r"J:\979个湖泊处理\Landsat5-Chla反演" 
        ...
        if son_file_names == []:
            ...
            for i in son_file_names: 
                ...
                if (HZ == ".tif"):
                    ...
                    Swir = data.GetRasterBand(5).ReadAsArray().astype(np.float32)
                    Swir1 = data.GetRasterBand(6).ReadAsArray().astype(np.float32)

                    
                    Rrs_data = pd.DataFrame({'Blue':Blue.reshape(-1),
                    ...
                    B1 = Rrs_data_cpu.loc[:, 'Blue'].values.reshape(-1)
                    B2 = Rrs_data_cpu.loc[:, 'Green'].values.reshape(-1)
                    B3 = Rrs_data_cpu.loc[:, 'Red'].values.reshape(-1)
                    B4 = Rrs_data_cpu.loc[:, 'Nir'].values.reshape(-1)
                    B5 = Rrs_data_cpu.loc[:, 'Swir'].values.reshape(-1)
                    B6 = Rrs_data_cpu.loc[:, 'Swir1'].values.reshape(-1)
                    ...
                    FAI = pd.DataFrame(
                        {'Rrc(NIR)-RrcL(NIR)': B4 - (B3 + (B5 - B3) * (859 - 645) / (1240 - 645))})  
                    ...
                    Chla = pd.DataFrame({"Ln(Chla)": (B2 - B1) / (B2 + B1)})

                    Chla_result = Chla.multiply(5.7672).add(0.1045)
